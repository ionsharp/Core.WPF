...
public static class XExplorer
{
    ...
    public static IValueConverter LegacyToolTipConverter = new ValueConverter<string, string>(i =>
    {
    ...
    static object OnPathCoerced(DependencyObject i, object value)
    {
        if (i is IExplorer j)
        {
            var oldPath = j.Path.NullOrEmpty() ? (GetDefaultPath(j) ?? Explorer.DefaultPath) : j.Path;
            var newPath = value?.ToString();

            if (!Folder.Long.Exists(newPath))
            {
                if (newPath != StoragePath.Root)
                {
                    return oldPath;
                    if (GetInvalidPathAlert(j))
                        Dialog.ShowError(GetInvalidPathAlertTitle(j), new Analytics.Error(GetInvalidPathAlertMessage(j)?.F(newPath)), Buttons.Ok);
                }
            }
            return newPath;
        }
