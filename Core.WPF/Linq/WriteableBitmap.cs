...
/// <summary>Extension methods for <see cref="System.Windows.Media.Imaging.WriteableBitmap"/>.</summary>
public static partial class XBitmap
{
    ...
    /// <summary>Clones the given <see cref="System.Windows.Media.Imaging.WriteableBitmap"/>.</summary>
    /// <param name="input">The <see cref="System.Windows.Media.Imaging.WriteableBitmap"/>.</param>
    /// <returns>A copy of the <see cref="System.Windows.Media.Imaging.WriteableBitmap"/>.</returns>
    public static WriteableBitmap Clone(this WriteableBitmap input)
    {
        using var source = input.GetBitmapContext(ReadWriteMode.ReadOnly);
        var result = New(source.Width, source.Height);
        using (var destination = result.GetBitmapContext())
        {
    ...
    public static Matrix<Vector4> Convert(this WriteableBitmap input)
    {
        if (input != null)
        {
            ...
            {
                result.SetValue(y.UInt32(), x.UInt32(), new Vector4(M.Normalize(color.R), M.Normalize(color.G), M.Normalize(color.B), M.Normalize(color.A)));
                return color;
            });
            return result;
        }
    ...
    unsafe public static void ForEach(this WriteableBitmap bmp, Func<int, int, Color> func)
    {
    ...
    unsafe public static void ForEach(this WriteableBitmap bmp, Func<int, int, Color, Color> func)
    {
        using var context = bmp.GetBitmapContext();
        var pixels = context.Pixels;
        var w = context.Width;
    ...
    unsafe public static Color GetPixel(this WriteableBitmap bmp, int x, int y)
    {
        using var context = bmp.GetBitmapContext(ReadWriteMode.ReadOnly);
    ...
    unsafe public static void SetPixeli(this WriteableBitmap bmp, int index, Color color)
    {
        using var context = bmp.GetBitmapContext();
    ...
    /// <param name="y">The y coordinate (column).</param>
    /// <param name="color">The color.</param>
    unsafe public static void SetPixel(this WriteableBitmap bmp, int x, int y, Color color)
    {
        using var context = bmp.GetBitmapContext();
    ...
    /// <param name="a">The alpha value of the color.</param>
    /// <param name="color">The color.</param>
    unsafe public static void SetPixeli(this WriteableBitmap bmp, int index, byte a, Color color)
    {
        using var context = bmp.GetBitmapContext();
    ...
    /// <param name="a">The alpha value of the color.</param>
    /// <param name="color">The color.</param>
    unsafe public static void SetPixel(this WriteableBitmap bmp, int x, int y, byte a, Color color)
    {
        using var context = bmp.GetBitmapContext();
        // Add one to use mul and cheap bit shift for multiplicaltion
        var ai = a + 1;
