...
public unsafe static partial class XBitmap
{
    ...
    private static List<int> ComputeBezierPoints(int x1, int y1, int cx1, int cy1, int cx2, int cy2, int x2, int y2, int color, Paint.BitmapContext context, int w, int h)
    {
    ...
    public static void Blend(this WriteableBitmap a, BlendModes blendMode, WriteableBitmap b, Point point, double opacity = 1)
    {
        a.ForEach((x0, y0, color) =>
        {
            if (x0 >= point.X && y0 >= point.Y)
            {
                ...
                var x2 = Clamp(x1, b.PixelWidth).Int32();
                var y2 = Clamp(y1, b.PixelHeight).Int32();
    ...
    static bool Similar(Color a, Color b, byte tolerance)
    {
        if (tolerance == 0)
            return a == b;

        var ab = a.GetBrightness();
        var bb = b.GetBrightness();

        var cb = Abs(ab - bb) * 255f;
        return cb <= tolerance;
    }

    static bool Similar(Color a, System.Drawing.Color b, byte tolerance)
    {
    ...
    public static void BlendAt(this WriteableBitmap input, BlendModes blendMode, System.Drawing.Point point, System.Drawing.Color fill, double opacity, byte tolerance)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            {
                input.SetPixel(p.X, p.Y, c.Blend(fillClone, blendMode, opacity));
                queue.Enqueue(p);
            }
    ...
    {
        var result = b.Transform(i => System.Windows.Media.Colors.Black.A(i));
        a.ForEach((x0, y0, color) =>
        {
            if (x0 >= point.X && y0 >= point.Y)
            {
                ...
                var x2 = Clamp(x1, (int)b.Columns).Int32();
                var y2 = Clamp(y1, (int)b.Rows).Int32();

                if (x1 < b.Columns && y1 < b.Rows)
                    return color.A(a => (Clamp((a / 255) - (b[y2, x2] / 255), 1) * 255).Byte());
            }
            return color;
        });
    }
    ...
    private static List<int> ComputeSegmentPoints(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float tension, int color, Paint.BitmapContext context, int w, int h)
    {
