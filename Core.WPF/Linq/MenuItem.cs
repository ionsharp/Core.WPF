...
public static class XMenuItem
{
    ...
    public static MenuItem New(string header, SmallImages? image = null, ICommand command = null, params FrameworkElement[] items)
    {
        var result = new MenuItem()
        {
            Command = command,
            Header = header,
            Icon = image != null ? Resource.GetUri($"Images/{image}.png", AssemblyType.Core) : null
        };

        items.If(i => i.Length > 0, i => i.ForEach(i => result.Items.Add(i)));
        return result;
    }

    public static MenuItem New(string header, params FrameworkElement[] items) => New(header, null, null, items);
