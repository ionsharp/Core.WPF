...
public class ColorModelEffect : BaseEffect
{
    ...
    public ColorModelEffect() : base()
    {
        this.Bind(ActualModelProperty,
            $"{nameof(Model)}", this, BindingMode.OneWay, new ValueConverter<Type, double>(i => Colour.GetIndex(i)));
        this.Bind(ActualModeProperty,
            $"{nameof(Mode)}", this, BindingMode.OneWay, new ValueConverter<Modes, double>(i => (int)i));
        this.Bind(ActualShapeProperty,
            $"{nameof(Shape)}", this, BindingMode.OneWay, new ValueConverter<Shapes2, double>(i => (int)i));
    ...
    System.Windows.Media.Media3D.Point3D GetPoint(Matrix m, int y) => new(m[y][0], m[y][1], m[y][2]);

    protected virtual void OnProfileChanged(ReadOnlyValue<WorkingProfile> input)
    {
        SetCurrentValue(CompandingProperty,
            input.New.Compression.GetAttribute<IndexAttribute>().Index.Double());

        if (input.New.Compression is Compression compression)
        {
        ...
        else if (input.New.Compression is GammaCompression gammaCompression)
        {
            SetCurrentValue(Compression_AProperty,
