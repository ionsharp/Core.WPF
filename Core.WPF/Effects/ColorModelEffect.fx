...
float TransferInverse(float input)
{
	...
	{
		float V = input;
		float L = V < BetaInverse ? V / 4.5 : pow((V + Alpha - 1) / Alpha, 1 / 0.45);
		return L;
	}
...
float Transfer(float input)
{
	...
	{
		float v = input;
		float V = v <= Epsilon ? v * Kappa / 100 : 1.16 * pow(v, 1 / 3) - 0.16;
		return V;
	}
	...
	{
		float L = input;
		float V = L < 0.018 ? 4.5 * L : 1.099 * pow(L, 0.45) - 0.099;
		return V;
	}
	...
	{
		float L = input;
		float V = L < Beta ? 4.5 * L : Alpha * pow(L, 0.45) - (Alpha - 1);
		return V;
	}
