...
[Name("File"), Serializable, View(MemberView.Tab, typeof(Tab))]
public class File : Item
{
    enum Tab { Contents, Exif }
    ...
    [Pin(Pin.AboveOrLeft), ReadOnly, StringStyle(StringStyle.FilePath)]
    public override string Path
    {
        get => base.Path; set => base.Path = value;
    }

    [Tab(Tab.Exif), Name("Properties")]
    public ExifTagCollection ExifProperties { get => Get<ExifTagCollection>(); private set => Set(value); }

    ///

    public File(string path) : base(ItemType.File, Origin.Local, path)
    {
        contents = new(RefreshContents, true);
        ExifProperties = new(path);
    }
    ...
    async Task RefreshContents(CancellationToken token)
    {
        ...
        await Task.Run(() =>
        {
            ...
            var result = Try.Invoke(() =>
            {
                ...
                fileText = Long.ReadAllText(Path, Encoding.Unicode);
            },
            e => Log.Write<File>(e));
            ...
            Dispatch.Invoke(() =>
            {
                ...
                Try.Invoke(() => { Lines += fileText.Lines(); Words += fileText.Words(); }, e => Log.Write<File>(e));
            });
        }, 
        token);
    ...
    public static void QuickRename(string path, bool recursive = false)
    {
        var files = Folder.Long.GetFiles(path);

        var j = 0;
        for (var i = files.Count() - 1; i >= 0; i--, j++)
        {
            var oldPath = files.ElementAt(i);

            var x = System.IO.Path.GetDirectoryName(oldPath);
            var y = System.IO.Path.GetExtension(oldPath).TrimStart(Array<char>.New('.')).ToLower();

            Func<string> z = new(() => $@"{x}\{j}.{y}");
            ...
            Try.Invoke(() => File.Long.Move(oldPath, newPath), e => { });
        }

        if (recursive)
            Folder.Long.GetFolders(path).ForEach(i => QuickRename(i, recursive));
    }
