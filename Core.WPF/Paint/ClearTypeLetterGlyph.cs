...
namespace Imagin.Core.Media
{
    public class ClearTypeLetterGlyph
    {
        ...
        public static ClearTypeLetterGlyph CreateGlyph(GlyphTypeface glyphTypeface, System.Drawing.Font font, double size, char ch, System.Windows.Media.Color fontColor, System.Windows.Media.Color bgColor)
        {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        if (color != bg2)
                        {
                            res.Add(new Item
                            {
                                X = (short)x,
                                Y = (short)y,
                                Color = XBitmap.ConvertColor(System.Windows.Media.Color.FromArgb(color.A, color.R, color.G, color.B)),
                            });
                        }
                    }
                }
            ...
            //    int bgint = XBitmap.ConvertColor(Color.FromArgb(bgColor.A, bgColor.R, bgColor.G, bgColor.B));
            //    for (int y = 0; y < height; y++)
            //    {
            ...
            //        {
            //            var color = bmp.GetPixel(x, y);
            //            line.Add(XBitmap.ConvertColor(Color.FromArgb(color.A, color.R, color.G, color.B)));
            //        }
