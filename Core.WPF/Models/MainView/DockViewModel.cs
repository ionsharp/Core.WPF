...
namespace Imagin.Core.Models
{
    public abstract class DockViewModel<View, Document> : MainViewModel<View>, IDockViewModel where View : MainWindow where Document : Models.Document
    {
        ...
        void OnDocumentPropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            switch (e.PropertyName)
            {
                case nameof(Models.Document.IsModified):
                    if (sender is Document document && document.IsModified)
                        Get.Where<DockViewOptions>().If(i => i.AutoSaveDocuments, i => document.Save());
