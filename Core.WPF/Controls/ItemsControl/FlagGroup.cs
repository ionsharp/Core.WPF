...
public class FlagGroup : ItemsControl
{
    ...
    static void OnValueChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e) => sender.As<FlagGroup>().OnValueChanged(e.Convert<Enum>());
    ...
    void Update()
    {
        ...
        {
            var result = new BaseCheckable<Enum>(i) { IsChecked = Value?.HasFlag(i) ?? false };
            items.Add(result);
        });
