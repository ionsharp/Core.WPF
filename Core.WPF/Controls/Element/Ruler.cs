...
public class Ruler : FrameworkElement
{
    ...
    public static readonly DependencyProperty UnitProperty = DependencyProperty.Register("Unit", typeof(Unit), typeof(Ruler), new FrameworkPropertyMetadata(Unit.Pixel, FrameworkPropertyMetadataOptions.AffectsRender));
    public Unit Unit
    {
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        for (double i = 0; i < Length; i += SmallStep)
        {
            ...
            {
                var formattedText = new FormattedText(Unit.Pixel.Convert(Unit, i + CountShift, Resolution.Double()).ToString(StringFormat, CultureInfo.CurrentCulture) + $" {Unit.GetAttribute<AbbreviationAttribute>().Abbreviation}", CultureInfo.CurrentCulture, FlowDirection.LeftToRight, new Typeface("Arial"), 6, Foreground, VisualTreeHelper.GetDpi(this).PixelsPerDip);
