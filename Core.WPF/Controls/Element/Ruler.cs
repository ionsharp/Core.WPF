    ...
    public class Ruler : FrameworkElement
    {
        ...
        protected override void OnRender(DrawingContext drawingContext)
        {
            ...
            for (double i = 0; i < Length; i += SmallStep)
            {
                ...
                {
                    var formattedText = new FormattedText((i + CountShift).Convert(Unit.Pixel, Unit, Resolution.Double()).ToString(StringFormat, CultureInfo.CurrentCulture) + $" {Unit.GetAttribute<AbbreviationAttribute>().Abbreviation}", CultureInfo.CurrentCulture, FlowDirection.LeftToRight, new Typeface("Arial"), 6, Foreground, VisualTreeHelper.GetDpi(this).PixelsPerDip);
