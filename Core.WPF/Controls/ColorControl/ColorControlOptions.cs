...
public class ColorControlOptions : BaseSavable, IColorControlOptions
{
    ...
    public int DefaultLayout => (int)LayoutNames.Explore;

    public IEnumerable<Uri> GetDefaultLayouts()
    {
        yield return Resources.Uri(AssemblyProperties.Name, $"{nameof(Controls)}/{nameof(ColorControl)}/Layouts/{LayoutNames.All}.xml");
        ...
        yield return Resources.Uri(AssemblyProperties.Name, $"{nameof(Controls)}/{nameof(ColorControl)}/Layouts/{LayoutNames.Select}.xml");
    }
    ...
    [NonSerialized]
    GroupWriter<MatrixModel> matrices;
    [Hidden]
    public GroupWriter<MatrixModel> Matrices
    {
        get => matrices;
    ...
    public void OnLoaded(ColorControl colorPicker)
    {
        ...
        Matrices = new GroupWriter<MatrixModel>($@"{Config.ApplicationProperties.GetFolderPath(Config.DataFolders.Documents)}\{nameof(ColorControl)}", "Matrices", "data", "matrices", new Collections.Limit(250, Collections.Limit.Actions.RemoveFirst));
        result = Matrices.Load();

        if (!result)
        {
            Matrices.Add(new Collections.ObjectModel.GroupCollection<MatrixModel>("LMS", 
                typeof(ChromaticAdaptationTransform).GetProperties().Where(i => i.Name != nameof(ChromaticAdaptationTransform.Default)).Select(i => new MatrixModel(i.GetDisplayName(), i.GetDescription(), (Matrix)i.GetValue(null)))));
        }
