...
public class Browser : Control, IExplorer
{
    ...
    readonly Method lengthTask;

    readonly Method<string[]> selectionTask;
    ...
    #region FileOpenedCommand

    public static readonly DependencyProperty FileOpenedCommandProperty = DependencyProperty.Register(nameof(FileOpenedCommand), typeof(ICommand), typeof(Browser), new FrameworkPropertyMetadata(null));
    ...
    #region FolderOpenedCommand

    public static readonly DependencyProperty FolderOpenedCommandProperty = DependencyProperty.Register(nameof(FolderOpenedCommand), typeof(ICommand), typeof(Browser), new FrameworkPropertyMetadata(null));
    public ICommand FolderOpenedCommand
    {
        get => (ICommand)GetValue(FolderOpenedCommandProperty);
        set => SetValue(FolderOpenedCommandProperty, value);
    }
    ...
    public static readonly DependencyProperty HistoryProperty = DependencyProperty.Register(nameof(History), typeof(StringHistory), typeof(Browser), new FrameworkPropertyMetadata(null));
    ...
    public Browser() : base()
    {
        ...
        lengthTask 
            = new Method(UpdateLength, true);
        selectionTask 
            = new Method<string[]>(null, UpdateSelectionLength, true);
    ...
    void OnSelected(object sender, NotifyCollectionChangedEventArgs e)
        => _ = selectionTask.Start((sender as ICollectionChanged).Select<Item, string>(i => i.Path).ToArray());
    ...
    protected virtual void OnFileOpened(string filePath)
    {
        ...
        FileOpenedCommand?.Execute(filePath);
    }
    ...
    { 
        ...
        FolderOpenedCommand?.Execute(folderPath);
    }
    ...
    public ICommand RenameCommand => renameCommand ??= new RelayCommand<Reference2>(i =>
    {
        if (i.First is TextBox box)
        {
            if (i.Second is Item item)
            {
                ...
                if (result is Error)
                {
                    ...
                    Dialog.ShowError("Rename", new Error($"Renaming failed!"), Buttons.Ok);
                }
            }
        }
    });
