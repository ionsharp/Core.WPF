...
namespace Imagin.Core.Controls
{
    public class SingleUpDown : NumericUpDown<float>
    {
        ...
        protected override object OnMaximumCoerced(object input) => input.As<float>().Clamp(AbsoluteMaximum, Value);

        protected override object OnMinimumCoerced(object input) => input.As<float>().Clamp(Value, AbsoluteMinimum);

        protected override object OnValueCoerced(object input) => input.As<float>().Clamp(Maximum, Minimum);
