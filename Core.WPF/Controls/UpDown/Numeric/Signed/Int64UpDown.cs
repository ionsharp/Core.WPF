...
namespace Imagin.Core.Controls
{
    public class Int64UpDown : NumericUpDown<long>
    {
        ...
        protected override object OnMaximumCoerced(object input) => input.As<long>().Clamp(AbsoluteMaximum, this.Value);

        protected override object OnMinimumCoerced(object input) => input.As<long>().Clamp(this.Value, AbsoluteMinimum);

        protected override object OnValueCoerced(object input) => input.As<long>().Clamp(Maximum, Minimum);
