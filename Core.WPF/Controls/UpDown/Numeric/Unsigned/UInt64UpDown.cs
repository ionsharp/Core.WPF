...
namespace Imagin.Core.Controls
{
    public class UInt64UpDown : NumericUpDown<ulong>
    {
        ...
        protected override object OnMaximumCoerced(object input) => input.As<ulong>().Clamp(AbsoluteMaximum, this.Value);

        protected override object OnMinimumCoerced(object input) => input.As<ulong>().Clamp(this.Value, AbsoluteMinimum);

        protected override object OnValueCoerced(object input) => input.As<ulong>().Clamp(Maximum, Minimum);
